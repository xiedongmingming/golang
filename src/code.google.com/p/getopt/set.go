...
type State int // a termination says why getopt returned.
...
type Set struct { // 表示选项集
	State                           // state of getopt
	args         []string           // args are the parameters remaining after parsing the optoins.
	program      string             // program is the name of the program for usage and error messages. If not set it will automatically be set to the base name of the first argument passed to parse.
	parameters   string             // 使用说明中的常量
	usage        func()             // usage should print the programs usage and exit.
	shortOptions map[rune]*option   // 短选项
	longOptions  map[string]*option // 长选项
	options      optionList         // 所有选项
}

func New() *Set { // returns a newly created option set.
...
var CommandLine = New() // The default set of command-line options.

func PrintUsage(w io.Writer) { CommandLine.PrintUsage(w) } // PrintUsage calls PrintUsage in the default option set.

func Usage() { CommandLine.usage() } // Usage calls the usage function in the default option set.
